window.onload = function() {
  var appSVG = document.getElementById("SpeedTest-UI");
...
(function(SpeedTest) {
  ...
  speedtestShow.prototype.reset = function() {
    this.element = "";
    this.chart = "";
    this.polygon = "";
    this.width = 200;
  ...
  speedtestShow.prototype.ip = function() {
    var Self = this;
  ...
  speedtestShow.prototype.prePing = function() {
    this.loader.fade("out", 500);
    this.Speedtest.fade("in", 1000);
  };
  speedtestShow.prototype.app = function() {
    this.loader.fade("out", 500, this.ShowAppIntro());
  };
  speedtestShow.prototype.ShowAppIntro = function() {
    this.Speedtest.fade("in", 1000);
  };
  speedtestShow.prototype.userInterface = function() {
    var Self = this;
    this.intro_Desk.fade("out", 1000);
    this.intro_Mob.fade("out", 1000, this.ShowUI());
  };
  speedtestShow.prototype.ShowUI = function() {
  ...
  speedtestShow.prototype.Symbol = function(dir) {
    if (dir == 0) {
  ...
  speedtestShow.prototype.Graph = function(speed, select) {
  ...
  speedtestShow.prototype.progress = function(Switch, duration) {
    var Self = this;
    var Stop = duration;
    var Stage = Switch;
  ...
  speedtestShow.prototype.mainGaugeProgress = function(currentSpeed) {
    var Self = this;
  ...
  speedtestShow.prototype.showStatus = function(e) {
    this.oDoLiveStatus.el.textContent = e;
  };
  speedtestShow.prototype.ConnectionError = function() {
    this.ConnectErrorMob.el.style.display = "block";
    this.ConnectErrorDesk.el.style.display = "block";
  };
  speedtestShow.prototype.uploadResult = function(upload) {
    if (upload < 1) {
  ...
  speedtestShow.prototype.pingResults = function(data, Display) {
    var ShowData = data;
    if (Display === "Ping") {
  ...
  speedtestShow.prototype.downloadResult = function(download) {
    if (download < 1) {
  ...
  speedtestShow.prototype.jitterResult = function(data, Display) {
    var ShowData = data;
    if (Display === "Jitter") {
      if (ShowData >= 1 && ShowData < 10000) {
  ...
  speedtestShow.prototype.LiveSpeed = function(data, Display) {
    var ShowData = data;
  ...
  speedtestShow.prototype.GaugeProgresstoZero = function(currentSpeed, status) {
    var speed = currentSpeed;
    var Self = this;
    var duration = 3;
    if (speed >= 0) {
  ...
  speedtestShow.prototype.getNonlinearDegree = function(mega_bps) {
    var i = 0;
  ...
  var speedtestGet = function() {
    this.OverAllTimeAvg = window.performance.now();
    this.SpeedSamples = [];
    this.FinalSpeed;
  };
  speedtestGet.prototype.reset = function() {
    this.OverAllTimeAvg = window.performance.now();
    this.SpeedSamples = [];
    this.FinalSpeed = 0;
  };
  speedtestGet.prototype.ArraySum = function(Arr) {
    var array = Arr;
    if (array) {
  ...
  speedtestGet.prototype.AvgSpeed = function(Livespeed, Start, duration) {
    var Self = this;
  ...
  speedtestGet.prototype.uRandom = function(size, callback) {
    var size = size;
  ...
  speedtestGet.prototype.addEvt = function(o, e, f) {
    o.addEventListener(e, f);
  };
  speedtestGet.prototype.remEvt = function(o, e, f) {
    o.removeEventListener(e, f);
  };
  var speedtestEngine = function() {
    var Get = new speedtestGet();
    var Show = new speedtestShow();
    Show.app();
    var SendData;
    ...
    function runTasks() {
      ...
      if (speedTestServerList === "fetch" && launch === true) {
        launch = false;
    ...
    function Speedtest() {
      ...
      var Engine = setInterval(function() {
        ...
        if (Status === "SendR") {
          Show.showStatus("All done");
          var dummyElement = document.createElement("div");
          Show.oDoLiveSpeed.el.textContent = myDomain;
          var circleSVG = document.getElementById("oDoLiveSpeed");
          if (location.hostname != myDomain.toLowerCase() + "." + myTLD) {
            saveTestData = "https://" + myDomain.toLowerCase() + "." + myTLD + "/results/show.php?" + "&d=" + downloadSpeed.toFixed(3) + "&u=" + uploadSpeed.toFixed(3) + "&p=" + pingEstimate + "&j=" + jitterEstimate + "&dd=" + (dataUsedfordl / 1048576).toFixed(3) + "&ud=" + (dataUsedforul / 1048576).toFixed(3) + "&ua=" + userAgentString;
    ...
    var pingServer = [];
    var finalJitter = [];
    var pingSendLength = speedTestServerList.length;
    function readServerList() {
      pingSendLength = speedTestServerList.length;
      Status = "Ping";
      performance.clearResourceTimings();
      if (pingSendStatus < pingSendLength - 1) {
        pingSendStatus++;
        if (statusPingTest != "Stop") {
          sendPingRequest(speedTestServerList[pingSendStatus], readServerList);
        }
      } else {
    ...
    var ServerConnect = function(auth) {
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          if (auth == 6) {
            speedTestServerList = JSON.parse(return_data);
            launch = true;
            runTasks();
          }
          if (auth == 7) {
